...
from pos_encodings import PositionalEncoding2D
from torch.utils.data import DataLoader, Dataset
from transformers import GPT2Config, GPT2LMHeadModel


class GPTLayer(GPT2LMHeadModel):
...
if __name__ == "__main__":
    ...
    ds = GPTDataset(top_ids)
    dl = DataLoader(ds, batch_size=8, shuffle=True)

    model = GPTLayer(
        GPT2Config(vocab_size=514, n_positions=1026, n_embd=120, side=32)
    ).to(device)
    ...
    for batch in tqdm(dl):
        ...
        print(batch.shape)
        loss,_ = model(input_ids=batch.to(device), labels=batch.to(device))
        loss.backward()
        optimizer.step()